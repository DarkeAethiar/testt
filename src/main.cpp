...
okapi::Motor dmmr(3, true, okapi::AbstractMotor::gearset::green, okapi::AbstractMotor::encoderUnits::degrees);
okapi::Motor dmbr(4, false, okapi::AbstractMotor::gearset::green, okapi::AbstractMotor::encoderUnits::degrees);
okapi::Motor dmfl(18, false, okapi::AbstractMotor::gearset::green, okapi::AbstractMotor::encoderUnits::degrees);
okapi::Motor dmml(5, true, okapi::AbstractMotor::gearset::green, okapi::AbstractMotor::encoderUnits::degrees);
okapi::Motor dmbl(6, false, okapi::AbstractMotor::gearset::green, okapi::AbstractMotor::encoderUnits::degrees);
//motor lr is lift.
okapi::Motor lr(9, true, okapi::AbstractMotor::gearset::green, okapi::AbstractMotor::encoderUnits::degrees);
...
void initialize()
{
  ...
  platpls.reset();
  pros::lcd::initialize();
  pros::lcd::set_text(1, "I am the BEEFSTER and ur bouta be BEEFED");
  //glp.set_value(false);
  //fgl.set_value(false);
...
/*
  dmfr.setBrakeMode(okapi::AbstractMotor::brakeMode::coast);
  dmfmr.setBrakeMode(okapi::AbstractMotor::brakeMode::coast);
  dmbmr.setBrakeMode(okapi::AbstractMotor::brakeMode::coast);
  dmbr.setBrakeMode(okapi::AbstractMotor::brakeMode::coast);
  dmfl.setBrakeMode(okapi::AbstractMotor::brakeMode::coast);
  dmfml.setBrakeMode(okapi::AbstractMotor::brakeMode::coast);
  dmbml.setBrakeMode(okapi::AbstractMotor::brakeMode::coast);
  dmbl.setBrakeMode(okapi::AbstractMotor::brakeMode::coast);
  */
}
void disabled() {}
...
void Abalance(){//probly dont work
  platpls.reset();
...
void AutonRight(){//when doing backwards movement do not tell the distance rather do negitive motor velocity.
glp.set_value(false);
fgl.set_value(true);
fts.set_value(true);
...
void Autonsuperultimatewinpointthatmadeethanwanttohanghimself(){//right auton two goal.
glp.set_value(false);
fgl.set_value(true);
fts.set_value(true);
Adrive(2,50,50);
...
void AutonSkills(){//left go to mid auton.
  glp.set_value(false);
  fgl.set_value(true);
...
void autonomous(){//BOTH CLAW AND LIFT MUST BE ON THE GROUND IF IT IS NOT IT WILL BREAK.
//std::cout << "===============" << std::endl;
//selection::execute();
//AutonRight();
//AutonLeft();
...
*/
  ...
  {
    ldrib.moveVelocity(con.getAnalog(okapi::ControllerAnalog::rightY) * -200);
  }
  ...
  {
    rdrib.moveVelocity(con.getAnalog(okapi::ControllerAnalog::leftY) * 200);
  }
}
...
/*
to upload type pros mu. Insure that file is saved before uploading.

bool flashers = true;
if(SensorValue(BS)==1){
 flashers=!flashers;
}
if(flashers=true&&SensorValue(BS)==1){
  turnonled(red);
  turnoffled(green);
}
else if(flashers=false&&SensorValue(BS)==1){
  turnoffled(red);
  turnonled(green);
}
else{
